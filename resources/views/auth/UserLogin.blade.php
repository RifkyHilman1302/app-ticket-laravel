...
@section('content')
    <div class="w-full flex flex-col items-center ">
        <p class="text-2xl mt-2 mb-2 font-bold">Sign In User</p>
        ...
        <form action="{{ route('user.loginSubmit') }}" method="POST" 
            ...
            <a 
                href="{{ route('user.regis') }}"
                class=" mt-5 mb-2 w-full h-10 border rounded text-black  text-sm flex items-center justify-center cursor-pointer hover:bg-blue-100">
                Create an account
            </a>
            <a 
                href="{{ route('admin.login') }}"
                class=" w-full h-10 border rounded text-black  text-sm flex items-center justify-center cursor-pointer hover:bg-blue-100">
                Sign In Admin?
            </a>
